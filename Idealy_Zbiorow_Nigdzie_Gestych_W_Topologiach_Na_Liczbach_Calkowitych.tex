...
\newcommand{\dummy}{{\bf dummy, dummy, dummy}}
...
\uncover<8->{
{\color{magenta}???} (jaka topologia?) $\Rightarrow$ $\cI$ ideał zbiorów nigdziegęstych w tej top.
}
\end{frame}
%----------------------------------
...
\end{frame}
%---------------------------------------------
\begin{frame}[label=lemCRT,shrink=1]\frametitle{Pomocniczy lemat}
\uncover<1->{
 \begin{block}{Lemat}
Niech $a,b,a_1,b_1,a_2,b_2 \in\N$. 
Jeśli $\arithseq{a_1}{b_1}\subseteq \arithseq{a}{b}$, $\arithseq{a_2}{b_2}\subseteq \arithseq{a}{b}$, 
i $\left(\frac{a_1}{a},\frac{a_2}{a}\right)=1$, to
$\arithseq{a_1}{b_1}\cap\arithseq{a_2}{b_2}\not=\emptyset$ (zatem jest ciągiem arytmetycznym). 
\end{block} }

\uncover<2->{
\begin{block}{Dowód}
\begin{itemize}
\item<3->Niech $f(n) = an+b$.  
\item<4-> Wówczas $f^{-1}[\arithseq{a_1}{b_1}] = \arithseq{\frac{a_1}{a}}{\frac{b_1-b}{a}}$,
$f^{-1}[\arithseq{a_2}{b_2}] = \arithseq{\frac{a_2}{a}}{\frac{b_2-b}{a}}$
a skoro $\left(\frac{a_1}{a},\frac{a_2}{a}\right)=1$
to stosując Chińskie twierdzenie o resztach, dostajemy że ich przekrój jest niepusty.
\item<5-> Więc
...
\end{frame}
%---------------------------------------------
\begin{frame}[shrink=5]\frametitle{Wynik ,,pozytywny''}
\uncover<1->{
...
i $b' := b \mod a'$ 
to wówczas, $\arithseq{a'}{b'} \in \B_K$
(bo $a' \in \SqrFr$, $(a',b') = 1$ gdyż $(a,b) = 1$ i $b'<a'$).}
\uncover<6->{Niech teraz $\arithseq{c'}{d'} \subseteq \arithseq{a'}{b'}$ będzie takie że $\arithseq{c'}{d'}\in \B_K$ 
(wówczas $c' \in \SqrFr$, $(c',d')=1$, $d'<c'$ i $a'\mid c'$).}
\uncover<7->{
Mamy $\arithseq{a}{b}\subseteq \arithseq{a'}{b'}$, $\arithseq{c'}{d'}\subseteq \arithseq{a'}{b'}$, 
i zauważmy że $\left(\frac{a}{a'}, \frac{c'}{a'}\right) = 1$ -- bo jedynym dzielnikiem pierwszym $\frac{a}{a'}$ 
są tylko te jakie się już pojawiły w rozkładzie  $a'$, 
podczas gdy wszystkie pierwsze dzielniki  $\frac{c'}{a'}$ 
muszą być różne od dzielników pierwszych $a'$ bo $c'\in\SqrFr$.
}
\uncover<8->{Zatem gdy zdefiniujemy $A:= \arithseq{a}{b}\cap\arithseq{c'}{d'} \neq \emptyset$ 
to $A \not=\emptyset$ (\hyperlink{lemCRT}{\beamergotobutton{lemat}}) i 
to będzie nasze szukane otoczenie,
bo  $A \in \T_G$ 
(jako przekrój dwóch zbiorów z $\T_G$) no i 
$A \subseteq \arithseq{a}{b}$. 
Z założenia wiemy że $X\cap A \neq \emptyset$. 
Mamy też $A \subseteq \arithseq{c'}{d'}$ a więc $X\cap \arithseq{c'}{d'} \neq \emptyset$.
}
\end{frame}
%---------------------------------------------
\begin{frame}[label=GFnotK,shrink=5]
\uncover<1->{
\begin{block}{Twierdzenie}
...
(gdy $C_k = \arithseq{a_k}{b_k}$ to $a_k$ jest parzysty i $\in\SqrFr$ i 
$\left(\frac{a_k}{2},\frac{2^k}{2}\right)=\left(\frac{a_k}{2},2^{k-1}\right)=1$ 
gdyż $\frac{a_k}{2}$ jest nieparzysty; oba $\arithseq{a_k}{b_k}$ i $\arithseq{2^k}{1}$ 
są podciągami $\arithseq{2}{1}$ zatem \hyperlink{lemCRT}{\beamergotobutton{lemat}}
zapewnia niepustość ich przekroju). 
...
Wówczas $V \not= \emptyset$ bowiem $(a,4) = 1$ zatem
$V\in \T_F$ i gdy $\arithseq{a}{b}\in \B_G$ to $V\in \T_G$.
Gdy $k > 1$ to $x_k \in \arithseq{4}{1}$ więc $V\cap X = \emptyset$.
...
$b$ jest nieparzyste. Gdy $b\not\equiv 1(\modulo 2^m)$, definiujemy
$V = \arithseq{a}{b}\setminus \{x_1,\ldots, x_{m-1}\}$.
Wówczas $\arithseq{a}{b}\cap \arithseq{2^m}{1} = \emptyset$ so $V\cap X = \emptyset$.

  Zatem możemy zakładać że $b\equiv 1(\modulo 2^m)$. Niech
\[
V = (\arithseq{a}{b} \cap \arithseq{2^{m+1}}{1 + 2^m}) \setminus \{x_1,\ldots, x_{m}\}.
\]
Wówczas $V \not= \emptyset$ gdyż $(\frac{a}{2^m} , \frac{2^{m+1}}{2^m}) = 1$
i $\arithseq{a}{b}$ i $\arithseq{2^{m+1}}{1 + 2^m}$
są zawarte w $\arithseq{2^m}{1}$. 
Gdy $k > m$ to $x_k \in \arithseq{2^{m+1}}{1}$ więc
$V\cap X = \emptyset$. Także $V\in\T_F$ i, gdy $\arithseq{a}{b} \in \B_G$ to
$V \in \T_G$.
}
...
\put(25,56){\makebox(0,0){{\color{magenta}?}}}
...
%---------------------------------------------
%---------------------------------------------
\begin{frame}\frametitle{Rola sekwencji $\arithseq{2}{2}$}

Pokażemy że dla dowolnego $\arithseq{a}{b} \in \B_K$ 
istnieje $\arithseq{c}{d} \in \B_K$ taki że $\arithseq{c}{d} \subseteq \arithseq{a}{b}$
oraz $\arithseq{c}{d} \cap \arithseq{2}{2} = \emptyset$. 
Mamy $(a,b)=1$, $b<a$ oraz $a \in \SqrFr$, rozważamy dwa przypadki:
\begin{enumerate}
 \item<2-> $2\mid b$. Wtedy $2 \nmid a$ (bo $(a:b)=1$). 
Niech $\arithseq{c}{d} := \arithseq{2a}{a+b}$. Skoro $(a,b)=1$ to $(a,a+b)=1$ i skoro
 $2 \nmid a$, mamy $(2a,a+b)=1$. 
Co więcej $2a\in\SqrFr$. Zatem $\arithseq{2a}{a+b}\in\B_K$, $\arithseq{2a}{a+b}\subseteq\arithseq{a}{b}$. Ponadto $\arithseq{2a}{a+b}\cap \arithseq{2}{2} = \emptyset$.

 \item<3-> $2\nmid b$. 
Gdy $2\mid a$ kładziemy $\arithseq{c}{d} := \arithseq{a}{b}$.
Gdy zaś $2\nmid a$ wtedy
kładziemy $\arithseq{c}{d} := \arithseq{2a}{b}$. Znów $2a\in\SqrFr$. Skoro $2\nmid b$ to $(2a,b)=1$. 
Więc, $\arithseq{2a}{b}\in\B_G$ i $\arithseq{2a}{b}\subseteq\arithseq{a}{b}$. 
Co więcej, $\arithseq{2a}{b}\cap \arithseq{2}{2} = \emptyset$.
\end{enumerate}
\end{frame}
%---------------------------------------------
%---------------------------------------------
...
\end{frame}
%---------------------------------------------
\begin{frame}[shrink=5]\frametitle{Nasze ideały są {\color{red}tall}}
\uncover<1->{
\begin{block}{Definicja}
...
\end{frame}
%------------------------------------------------------------------------------
\begin{frame}[shrink=5]\frametitle{Nasze ideały są {\color{red}tall} - c.d.}
\uncover<1->{
\begin{block}{Obserwacja}
\begin{itemize}
\item<2->
Standardowe bazy topologii Furstenberga, Golomba oraz Kircha
mają własność \emph{splittingu}. 
\item<3->Wynika to z twierdzenia:
  {\bf Twierdzenie} 
Każda baza topologii Hausdorffa bez
punktów izolowanych ma własność \emph{splittingu}.
\item<4->Dowód?...
\item<5->...W zasadzie jest rutynowy.
\end{itemize}
\end{block}
}
\uncover<6->{
...
\end{frame}
%---------------------------------------------
\begin{frame}[shrink=1,label=powrotMaz]\frametitle{Szkic dowodu poprzedniego twierdzenia}
\uncover<1->{
\begin{block}{Dowód}
Przypuśćmy że $\I=S^0(\mathcal{F})$ jest ideałem typu $F_\sigma$.
Charakteryzacja K.Mazura \hyperlink{bibliografia}{\beamergotobutton{publikacja}} 
powiada iż istnieje dolnie półciągła podmiara
$\phi\colon\mathcal{P}(\N)\to [0, \infty]$
, to znaczy taka funkcja iż
dla dowolnych $A,B\subseteq\N$:}
\begin{enumerate}
\item<2-> $\phi(\emptyset)=0$ oraz $\phi(\N)>0$,
\item<3-> $\phi(\{n\})<\infty$ dla każdego $n\in\N$,
\item<4-> $A\subseteq B \implies\phi(A)\leq \phi(B)$,
\item<5-> $\phi(A\cup B) \leq \phi(A) + \phi(B)$,
\item<6-> $\phi(A)=\lim_{n\to\infty} \phi(A \cap \{1,\ldots,n\})$ (\emph{dolna półciągłość}),
\end{enumerate}
\uncover<7->{
taka że $\I= \Fin(\phi)=\{A\subseteq \N :\ \phi(A)<\infty\}$.
}
\uncover<8->{
Ze splittingu ustalamy rodzinę $\{F_i\}_{i\in\N}$ parami rozlącznych elementów $\mathcal{F}$.
Mamy $\forall_{i}\ F_i\notin\I$, więc $\phi(F_i)=\infty$. Dla każdego $i\in\N$ wybieramy
(na mocy dolnej półciągłości $\phi$) skończony $A_i\subseteq F_i$ taki że $\phi(A_i)>i$. 
Definiujemy $A:=\bigcup_{i\in\N}{A_i}$. 
Można wykazać (pomijamy nietrudny dowód)
że $A\in S^0(\mathcal{F}) = \I$. Z drugiej strony $\forall_{i}\ A_i \subseteq A$ więc $i<\phi(A_i)\leq \phi(A)$. Zatem $\phi(A)=\infty$ więc $A\notin\I$, sprzeczność.}
\end{block}
\end{frame}
%---------------------------------------------
...
\end{frame}
%---------------------------------------------
\begin{frame}[label=powrotFMRS,label=powrotBFMS]\frametitle{Własność Bolzano-Weierstrassa}
\uncover<1->{
W artykule \hyperlink{bibliografia}{\beamergotobutton{publikacja}}, 
R. Filipów, N. Mrożek, I. Recław, oraz P. Szuca 
wprowadzili tak zwaną {\color{magenta} własność Bolzano-Weierstrassa} ideałów. 
}
\uncover<2->{
\begin{block}{Definicja}
Mówimy że ideał $\I$ na $\N$ 
ma {\color{magenta} własność $\finbw$} gdy dla dowolnego ograniczonego ciągu
$(x_n)_{n\in\N}$ liczb rzeczywistych istnieje $A\notin\I$ 
taki że podciąg $(x_n)_{n\in A}$ jest zbieżny.
\end{block}
}
\uncover<3->{
Istnieje wiele równoważnych i zdecydowanie bardziej wygodniejszych do 
zastosowań charakteryzacji własności $\finbw$, np. w pracy
\hyperlink{bibliografia}{\beamergotobutton{Barbarski P., Filipów R., Mrożek N., Szuca P.}}
jest charakteryzacja używająca pojęcia tzw. {\color{purple}drzewa zbiorów doskonałych}.
}
\end{frame}

\begin{frame}\frametitle{Pożyteczna charakteryzacja}

\uncover<1->{
\begin{block}{Twierdzenie [Marta Kwela \& AN]}
Ideał $\I$ {\color{red} nie ma} własności $\finbw$ wtedy i tylko wtedy gdy
istnieje ciąg skończonych podziałów $(\mathcal{P}_n)_{n\in\N}$ zbioru $\N$ takich, że
...
gdzie $A\subseteq^{*} B$ jest tak zwanym {\color{purple} prawie zawieraniem}, tzn.
$A\setminus B$ jest zbiorem skończonym).
}
\end{frame}

\begin{frame}\frametitle{W odmętach partycji...}
\uncover<1->{
\begin{block}{Twierdzenie}
Ideały $\I_F$, $\I_G$ oraz $\I_K$ {\color{red} nie mają} własności $\finbw$.
\end{block}}

\uncover<2->{
\begin{block}{Dowód}
Definiujemy odpowiedni ciąg skończonych partycji 
$(\mathcal{P}_k)_{k\in\N}$ zbioru $\N$ jak następuje:
...
\hyperlink{powrotBFMS}{\beamerreturnbutton{Powrót}}
...
\hyperlink{powrotFMRS}{\beamerreturnbutton{Powrót}}
...
\hyperlink{powrotMaz}{\beamerreturnbutton{Powrót}}
